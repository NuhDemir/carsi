...
if (!API_BASE_URL) {
  ...
  API_BASE_URL = `${proto}//${host}${API_BASE_URL}`;
}
...
const apiRequest = async (url, options = {}) => {
  const config = {
    headers: {
      ...
      // Eğer localStorage'da token varsa Authorization başlığı ekle
...
export const ProductProvider = ({ children }) => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      // Backend may return an envelope { success, data, ... } or plain array
      const productsPayload = data?.data ?? data;
      dispatch({ type: actionTypes.SET_PRODUCTS, payload: productsPayload });
    } catch (error) {
